...
class DSGGDataset(TvCocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if self.seq_sort:
            ...
            # anchor frame(현재 프레임)이 정렬 후 몇 번째 인덱스로 갔는지 계산
            cur_frame_new_idx = sorted_idx.index(0)
            
            if self.return_ref_targets:
                targets = [targets[i] for i in sorted_idx]
                targets[0]['cur_idx'] = torch.as_tensor([cur_frame_new_idx])
            else:
                target['cur_idx'] = torch.as_tensor([cur_frame_new_idx]) # sgdet task에서는 단일 target 변수만 존재함
        else:
            if self.return_ref_targets: # predcls:True / sgdet:False
...
class ResizeFixed(object):
    ...
    def __call__(self, image, target):
        ...
        # boxes는 좌표 스케일링이 필요한데 기존 RandomResize가 하던 방식이 있다면 같은 로직을 재사용해야 합니다.
        # 만약 기존 T.RandomResize가 target의 boxes를 자동으로 스케일링해줬다면,
        # 이 클래스도 동일하게 scale factor를 적용해 갱신하는 로직을 구현해야 합니다.
        if "boxes" in target:
