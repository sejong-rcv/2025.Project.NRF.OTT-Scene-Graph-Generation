"""
previous frame(t-1 frame)과 current frame(t frame)을 img에 채널 축으로 concat해서 return. 
target은 current frame(t frame)만 return
"""
"""
...
import datasets.transforms_multi_prev as T
...
class DSGGDataset(TvCocoDetection):
    def __init__(self, img_folder, ann_file, transforms, interval1, interval2, num_ref_frames=3, \
        ...
        print(' Build DSGG Multi-frame Dataset (returns pairs of t, t-1) ')
    ...
    def __getitem__(self, idx):
        ...
        현재 frame 기준으로 reference frame id를 뽑고, image list imgs를 [현재, ref1, ref2, ...]로 묶어 반환 
        Target Frame과 Reference Frames N장 (N+1) 뿐만 아니라, 각 Frame마다 previous frames을 함께 짝지어서 load
        """
        coco = self.coco
        img_id = self.ids[idx] # img_id == idx + 1
        ann_ids = coco.getAnnIds(imgIds=img_id) # ex) [8978, 8979, 8980, 8981, 8982]
        ...
        video_all_img_ids = self.cocovid.get_img_ids_from_vid(video_id)
        
        imgs = [] 
        
        # Target Image (t)
        path = img_info['file_name'] # ex) 'frames/0JA9E.mp4/000350.png'
        img = self.get_image(path)

        # Target Prev (t-1)
        if (img_id - 1) in video_all_img_ids:
            prev_img_id = img_id - 1
        else:
            prev_img_id = img_id
        
        prev_info = coco.loadImgs(prev_img_id)[0]
        img_prev = self.get_image(prev_info['file_name'])
        
        imgs.append(img)    # append to list [Target_t, Target_t-1]
        ...
        if self.return_ref_targets: # predcls:True / sgdet:False
            targets = []
            targets.append(target)
            
            
        # Reference Frames Sampling
        ref_img_ids = []
        if self.is_train:
            interval = self.interval1
            left = max(video_all_img_ids[0], img_id - interval)
            right = min(video_all_img_ids[-1], img_id + interval)
        ...
        else:   # eval : 균등 간격으로 num_ref_frames개를 뽑도록 설계(범위는 ± interval2)
        ...
        # Reference Frames & Prev Frames Load
        for ref_id in ref_img_ids:           
            # Ref (t)
            ref_info = coco.loadImgs(ref_id)[0]
            ref_img = self.get_image(ref_info['file_name'])
            
            # Ref Prev (t-1)
            if (ref_id - 1) in video_all_img_ids:
                ref_prev_id = ref_id - 1
            else:
                ref_prev_id = ref_id
            
            ref_prev_info = coco.loadImgs(ref_prev_id)[0]
            ref_prev_img = self.get_image(ref_prev_info['file_name'])
            
            # append to list [Ref_i_t, Ref_i_t-1]
            imgs.append(ref_img)
            imgs.append(ref_prev_img)

            if self.return_ref_targets:
                # ... (Ref target 처리 로직 동일) ...
                ref_ann_ids = coco.getAnnIds(imgIds=ref_id)
                ref_target_raw = coco.loadAnns(ref_ann_ids)
                ref_tgt = {'image_id': ref_id, 'annotations': ref_target_raw}
                _, ref_tgt = self.prepare(ref_img, ref_tgt)
                targets.append(ref_tgt)
...
class ResizeFixed(object):
    def __init__(self, size_hw):  # (H, W) tuple
        self.size_hw = size_hw
    def __call__(self, image, target):
        h, w = self.size_hw
        image = [ F.resize(img, [h, w]) for img in image ] # image list resize
        # image = F.resize(image, [h, w])  # 정확히 (H,W)
