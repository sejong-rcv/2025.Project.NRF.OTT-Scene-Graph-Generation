            ...
            if "masks" in tgt:
                if target['masks'].shape[0]>0:
                    tgt['masks'] = interpolate(
                        tgt['masks'][:, None].float(), size, mode="nearest")[:, 0] > 0.5
                else:
                    tgt['masks'] = torch.zeros((tgt['masks'].shape[0],h,w))
            targets.append(tgt)
...
class RandomCrop(object):
    ...
    def __call__(self, img, target):
        # [수정] img는 리스트이므로, 첫 번째 이미지를 기준으로 crop 파라미터를 구해야 함
        region = T.RandomCrop.get_params(img[0], self.size)
        return crop(img, target, region)
...
class ToTensor(object):
    def __call__(self, clip, target):
        # clip: List[PIL Image] -> List[Tensor]
        img = []
        for im in clip:
...
class Normalize(object):
    ...
    def __call__(self, clip, target=None):
        # clip: List[Tensor] -> List[Tensor] (Normalized)
        image = []
        for im in clip:
