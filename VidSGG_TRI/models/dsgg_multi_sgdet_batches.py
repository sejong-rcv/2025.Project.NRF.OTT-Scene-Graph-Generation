...
from .cdn_multi_sgdet_batches import build_cdn
...
class CDNDSGG(nn.Module):
    def __init__(self, backbone, transformer, num_obj_classes, num_queries, aux_loss=False, args=None):
        ...
        print('  [Info] Building Multi-Frame CDNDSGG with Multiple-Batch size')
    ...
    def forward(self, samples: NestedTensor, targets=None, cur_idx=0):
        ...
        # samples.tensors Shape: [Batch, (N_ref + 1) * 3, H, W]. (N_ref + 1) 개의 frame 쌍이 채널 방향으로 연결되어 있음.
        # [Frame0, Frame1, ...]        
        B, C_total, H, W = samples.tensors.shape
        num_frames = C_total // 3 # (N_ref + 1)
        
        # Prepare Mask: Expand [B, H, W] -> [B*N, H, W]
        mask = samples.mask.unsqueeze(1).repeat(1, num_frames, 1, 1).contiguous().view(B * num_frames, H, W)
                
        # 1. Batch Folding: Process ALL frames (Target + Refs) in parallel
        # Reshape: [B, N*3, H, W]=>[B, N, 3, H, W]=>[B*N, 3, H, W]. treats every frame (target or reference) as an independent sample in a large batch
        sample = samples.tensors.contiguous().view(B, num_frames, 3, H, W).reshape(B * num_frames, 3, H, W)
        
        samples = NestedTensor(sample, mask)

        features, pos = self.backbone(samples) # if ResNet, features Tensor shape : [12, 2048, 32, 18] / features mask shape : [12, 32, 18] / position encoding
        src, mask = features[-1].decompose()
        src = self.input_proj(src)
        assert mask is not None
        
        _, C_f, H_f, W_f = src.shape # [B*N, C, H_f, W_f]
        # Reshape: [B*N, ...] -> [B, N, ...] -> [N, B, ...]
        transformer_input = src.view(B, num_frames, C_f, H_f, W_f).permute(1, 0, 2, 3, 4)
        ...
        # # encoder, insance decoder, relation decoder, temporal_query_layer1~3
        # hopd_out, interaction_decoder_out = self.transformer(transformer_input, 
        #                                                      mask, self.query_embed.weight, 
        #                                                      pos[-1], embed_dict, targets, cur_idx)[:2]
        
        hopd_out, interaction_decoder_out, ins_attn_weights, rel_attn_weights, _, temporal_weights = self.transformer(
                                                            transformer_input, mask, 
                                                            self.query_embed.weight, 
                                                            pos[-1], embed_dict, targets, cur_idx)
        ...
        out = {'pred_obj_logits': outputs_obj_class, 'pred_sub_boxes': outputs_sub_coord, 
               'pred_obj_boxes': outputs_obj_coord, 'pred_attn_logits': outputs_attn_class, 
               'pred_spatial_logits': outputs_spatial_class, 'pred_contacting_logits': outputs_contacting_class,
               'ins_attn_weight': ins_attn_weights, 'rel_attn_weight': rel_attn_weights,
...
def build(args):
    ...
    weight_dict['loss_sub_bbox'] = args.bbox_loss_coef # L1
    weight_dict['loss_obj_bbox'] = args.bbox_loss_coef # L1
