...
class CDN(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1) # [bs,c,h,w]->[hw, bs,c]
        ...
        hopd_out, ins_attn_weight = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed)
        hopd_out = hopd_out.transpose(1, 2)
        
        if not self.one_dec: # True
            interaction_query_embed = hopd_out[-1] # [BS, Q_num, dim]
            interaction_query_embed = interaction_query_embed.permute(1, 0, 2) # [Q_num, BS, dim]
            ...
            interaction_decoder_out, rel_attn_weight = self.interaction_decoder(interaction_tgt, memory, memory_key_padding_mask=mask,
                                    pos=pos_embed, query_pos=interaction_query_embed)
            interaction_decoder_out = interaction_decoder_out.transpose(1, 2)
        else:
            interaction_decoder_out = hopd_out
            
            

        return hopd_out, interaction_decoder_out, ins_attn_weight.view(bs, self.num_queries, h, w), rel_attn_weight.view(bs, self.num_queries, h, w), memory.permute(1, 2, 0).view(bs, c, h, w)
        # return hopd_out, interaction_decoder_out, ins_attn_weight.view(bs, 100, h, w), rel_attn_weight.view(bs, 100, h, w), memory.permute(1, 2, 0).view(bs, c, h, w)
