...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, iou_threshold=0.5, save_file="tmp", constraint=False, semithreshold=None, error_analysis=False, nms_filter=False):
        ...
        self.print_cls_rec1 = True # other way
    ...
    def evaluate_scene_graph(self, gt, pred):
        '''collect and refine the groundtruth and prediction'''
        for idx, frame_gt in enumerate(gt ):# gt[i] indices all annotations for the i-th frame
            ...
            for obj_idx in range(len(frame_gt['obj_labels'])):
                gt_relations.append([human_idx, obj_idx+1, int(torch.where(frame_gt['attn_labels'][obj_idx])[0])]) # for attention triplet <human-object-predicate>
            ...
            # generate pred entry
            # pred[idx].keys() : dict_keys(['pred_scores', 'pred_labels', 'pred_boxes', 'pair_idx', 'attention_distribution', 'spatial_distribution', 'contacting_distribution'])
            # first part for attention and contact, second for spatial
            rels_i = np.concatenate((pred[idx]['pair_idx'],             # attention (person->object)
                                     pred[idx]['pair_idx'][:,::-1],     # spatial. (object->person). inverse 
                                     pred[idx]['pair_idx']), axis=0)    # contacting (person->objecgt)
            ...
            # gt_entry.keys() : dict_keys(['gt_classes', 'gt_relations', 'gt_boxes', 'img_path']
            # pred_entry.keys() : dict_keys(['pred_boxes', 'pred_classes', 'pred_rel_inds', 'obj_scores', 'rel_scores'])
    ...
    def evaluate_from_dict(self, gt_entry, pred_entry, mode, method=None, threshold = 0.9, num_rel=26, **kwargs):
        ...
        :param gt_entry: Dictionary containing gt_relations, gt_boxes, gt_classes.                      dict_keys(['gt_classes', 'gt_relations', 'gt_boxes', 'img_path'])
        :param pred_entry: Dictionary containing pred_rels, pred_boxes (if detection), pred_classes.    dict_keys(['pred_boxes', 'pred_classes', 'pred_rel_inds', 'obj_scores', 'rel_scores'])
        :param mode: 'sgdet', ..
        :param method: constraint ('with', 'no')
        :param result_dict:
        :param kwargs:
        :return:
        """
        ...
        pred_rel_inds = pred_entry['pred_rel_inds'] # (sub_idx, obj_idx) pair
        rel_scores = pred_entry['rel_scores']        # (num_pairs, num_predicate_classes)
        pred_boxes = pred_entry['pred_boxes'].astype(float)
        pred_classes = pred_entry['pred_classes']
        obj_scores = pred_entry['obj_scores']

        if method == 'no':
            # object confidence와 relation distributino 펼쳐 "pair, all predicate" 조합 만들고 상위 100개 유지
            obj_scores_per_rel = obj_scores[pred_rel_inds].prod(1)      # (sub, obj) score product
            overall_scores = obj_scores_per_rel[:, None] * (rel_scores) # [num_rows, 26].   ** 2
            score_inds = argsort_desc(overall_scores) # top100 (row, pred_id)
            ...
            # 각 행(페어)마다 argmax로 하나의 predicate 선택. (sub_id, obj_id, 단일 rel)형태로 행 수 = 3 x num_pairs (세 타입 모두 1개씩)
        ...
        for k in self.result_dict[mode + '_recall']:
            match = reduce(np.union1d, pred_to_gt[:k]) # top-k prediction이 커버한 GT index들 union
            ...
            rec_i = float(len(match)) / float(gt_rels.shape[0]) # recall@K of frame
...
###########################
    def evaluate_recall(self, gt_rels, gt_boxes, gt_classes,
        ...
        # relation_scores = [[score_sub_cls, score_obj_cls, score_rel], ...]
        sorted_scores = relation_scores.prod(1) # product 3 scores
        pred_triplets = pred_triplets[sorted_scores.argsort()[::-1],:] # sort
    ...
    def _compute_pred_matches(self, gt_triplets, pred_triplets,
        ...
        keeps = intersect_2d(gt_triplets, pred_triplets) # if True, (sub_cls, rel_id, obj_cls) matched
        ...
        for gt_ind, gt_box, keep_inds in zip(np.where(gt_has_match)[0],
                                            ...
                                            keeps[gt_has_match]):
            boxes = pred_boxes[keep_inds] # Only the ones that match gt. gt랑 매칭되는 것들만
            ...
            # inds = np.array(inds)[0] -> inds = np.asarray(inds, dtype=bool) ???
            ######################
