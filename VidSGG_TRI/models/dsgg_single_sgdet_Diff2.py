...
class CDNDSGG(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_queries, aux_loss=False, args=None):
        super().__init__()
        print('  build Temporal Feature Diff2 embedding module')
        ...
        # 1x1 conv to project feature dimension
        self.input_proj = nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)

        
        self.motion_logit = nn.Parameter(torch.tensor(-2.0))  # sigmoid(-2) ≈ 0.12 (초기엔 약하게)
        
        # Projection Layer for Motion Embedding
        # Backbone 채널(DINOv2 등)을 Transformer Hidden Dim으로 변환
        self.motion_encoder = MotionEncoder(in_channels=3, hidden_dim=hidden_dim)

        # Fusion Layer (Channel Compression(1x1) -> Spatial Refinement(3x3)). 
        # 1. Channel Mixing (1x1): 서로 다른 두 Modality(Semantic, Motion)를 결합
        # 2. Spatial Refinement (3x3): Interpolation으로 인한 공간적 오차 보정 및 Local Context 반영
        self.fusion_proj = nn.Sequential(
            # 1. Channel Compression & Interaction
            # 입력: [B, 2*hidden_dim, H, W] -> 출력: [B, hidden_dim, H, W]
            # 역할: Semantic 정보와 Motion 정보를 픽셀 단위로 섞고 차원을 축소합니다.
            nn.Conv2d(hidden_dim * 2, hidden_dim, kernel_size=1, bias=False),
            nn.GroupNorm(32, hidden_dim),
            nn.ReLU(inplace=True),

            # [Step 2] Spatial Context Refinement (3x3 Conv)
            # 입력: [B, hidden_dim, H, W] -> 출력: [B, hidden_dim, H, W]
            # 역할: 1x1 만으로는 볼 수 없는 '주변 문맥'을 파악합니다. 
            # 특히 DINO Feature(Patch 14)와 Motion Feature(Stride 16) 간의 
            # 미세한 격자 불일치(Grid Misalignment)를 Smoothing하여 Transformer에 전달합니다.
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        # 6채널 입력 처리 (t, t-1) 및 Feature Boosting
        # samples.tensors: [Batch, Channel, H, W] -> (t, t-1) stacked
        # 입력 분리 (t frame, t-1 frame)
        if samples.tensors.shape[1] == 6: 

            # Image Split
            t_tensors = samples.tensors[:, :3, :, :] # 채널 0-2: Current Frame (t)
            prev_tensors = samples.tensors[:, 3:, :, :]  # 채널 3-5: Previous Frame (t-1)
            mask = samples.mask 
            
            # Current Frame Backbone Forward e)
            samples_t = NestedTensor(t_tensors, mask)
            features_t, pos = self.backbone(samples_t)
            src_t, mask = features_t[-1].decompose() # [B, C_backbone, H_feat, W_feat]
            # t frame feature Projection & Fusion (Concatenation)
            src_projected = self.input_proj(src_t) # [B, hidden_dim, H_feat, W_feat]
            
            # 2. Raw Pixel Difference Calculation
            # Normalization이 되어 있는 상태이므로 그대로 뺌. 
            # (Motion은 절대적인 색상값이 아니라 변화량이므로 normalization된 값의 차이도 유효함)
            diff_img = t_tensors - prev_tensors # [B, 3, H, W]
            # 4. Motion Encoder Forward
            motion_feat = self.motion_encoder(diff_img) # [B, hidden_dim, H_motion, W_motion]
            # 5. Spatial Alignment (Interpolation)
            # Backbone Feature와 Motion Feature의 H, W가 미세하게 다를 수 있으므로 맞춤
            ...
            alpha = torch.sigmoid(self.motion_logit)  # (0,1)
            motion_feat = motion_feat * alpha # motion_feat learnable weight
            
            # Concat along channel dimension
            assert motion_feat.shape[-2:] == src_projected.shape[-2:], (motion_feat.shape, src_projected.shape)
            fused_feat = torch.cat([src_projected, motion_feat], dim=1) # [B, 2*hidden_dim, H, W]
            
            # Project back to hidden_dim
            transformer_input = self.fusion_proj(fused_feat) # [B, hidden_dim, H, W]
    
        else:
            # Single Frame Case (Fallback)
        ...
        hopd_out, interaction_decoder_out, ins_attn_weight, rel_attn_weight = \
            self.transformer(transformer_input, mask, self.query_embed.weight, pos[-1])[:4]
