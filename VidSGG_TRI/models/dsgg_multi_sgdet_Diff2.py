"""
t, t-1를 backbone 거치기 전에 Difference modeling 하도록 TRI module 수정. (기존에는 t, t-1 프레임 각각 ViT로 feature extraction 한 이후에 residual)
MotionEncoder (CNN)추가. 
Motion Embedding : (t-1, t) 텐서값 그대로 빼서 MotionEncoder 통과시켜서 Motion Embedding 생성
Motion Embedding Injection : t 백본 feature와 Motion Embedding concat 후 1x1 conv로 차원 맞춤

Key Features:
1. Batch Folding: Processes Target and Reference frames in parallel by reshaping [B, N, C, H, W] -> [B*N, C, H, W].
2. Diff2 Strategy: Uses Raw Pixel Difference (t - t_prev) and Motion Encoder.
3. Deep Fusion: Uses the optimized fusion projection (Concat -> 1x1 -> 3x3 -> Residual).
"""
...
###  실행 전 import 한 cdn_single_sgdet 파일 확인 
from .cdn_multi_sgdet_Diff import build_cdn # Transformer for Multi-Frames

from models.word_vectors import obj_edge_vectors
from util.label_set import OBJ_CLASSES


class ResBlock(nn.Module): # for MotionEncoder
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # Stride가 1이 아니거나 채널 수가 변할 경우 identity mapping(x)의 차원을 맞춰줘야 함
...
# Raw Pixel Difference를 입력받아 Backbone Feature Map 크기로 줄여주는 CNN
class MotionEncoder(nn.Module):
    def __init__(self, in_channels=3, hidden_dim=256):
        ...
        self.layer4 = ResBlock(256, hidden_dim, stride=2)
...
class CDNDSGG(nn.Module):
    def __init__(self, backbone, transformer, num_obj_classes, num_queries, aux_loss=False, args=None):
        super().__init__()
        print('  [Info] Building Multi-Frame CDNDSGG with Raw-Diff & Optimized Deep Fusion')
        self.num_queries = num_queries
        self.transformer = transformer
        hidden_dim = transformer.d_model

        # Query Embeddings & Heads
        ...
        # Learnable Gating Logit
        self.motion_logit = nn.Parameter(torch.tensor(-2.0)) 
        # Motion Encoder
        self.motion_encoder = MotionEncoder(in_channels=3, hidden_dim=hidden_dim)

        # Optimized Deep Fusion Projector (Channel Compression(1x1) -> Spatial Refinement(3x3))
        self.fusion_proj = nn.Sequential(
            # 1. Channel Mixing & Compression
            nn.Conv2d(hidden_dim * 2, hidden_dim, kernel_size=1, bias=False),
            nn.GroupNorm(32, hidden_dim),
            nn.ReLU(inplace=True),
            # 2. Spatial Refinement (Context Aggregation)
        ...
        if self.ins_temporal_embed_head: # if True, deepcopy spatial predictors to make temporal head
        ...
        # # Semantic Position Initialization (Optional)
    ...
    def forward(self, samples: NestedTensor, targets=None, cur_idx=0):
        ...
        # samples.tensors Shape: [Batch, (N_ref + 1) * 6, H, W]. (N_ref + 1) 개의 frame 쌍이 채널 방향으로 연결되어 있음.
        # [Frame0_t, Frame0_t-1, Frame1_t, Frame1_t-1, ...]        
        B, C_total, H, W = samples.tensors.shape
        num_frames = C_total // 6 # (N_ref + 1)
        
        # 1. Batch Folding: Process ALL frames (Target + Refs) in parallel
        # Reshape: [B, N*6, H, W]=>[B, N, 6, H, W]=>[B*N, 6, H, W]. treats every frame (target or reference) as an independent sample in a large batch
        flat_tensors = samples.tensors.contiguous().view(B, num_frames, 6, H, W).reshape(B * num_frames, 6, H, W)
        
        # Split Current(t) and Previous(t-1) from the 6 channels
        t_img = flat_tensors[:, :3, :, :]
        prev_img = flat_tensors[:, 3:, :, :]
        
        # Prepare Mask: Expand [B, H, W] -> [B*N, H, W]
        # Assuming all frames in a batch share the same mask size (resized to fixed size)
        mask_orig = samples.mask
        mask_flat = mask_orig.unsqueeze(1).repeat(1, num_frames, 1, 1).contiguous().view(B * num_frames, H, W)
        
        # 2. Parallel Feature Extraction (Semantics + Motion)
        # A. Semantic Feature (Backbone on t frame)
        samples_t = NestedTensor(t_img, mask_flat)
        features_t, pos_t = self.backbone(samples_t) # backbone forward
        src_t, mask_t = features_t[-1].decompose() # [B*N, C_bb, H_f, W_f]
        
        # Projection
        src_projected = self.input_proj(src_t) # [B*N, hidden_dim, H_f, W_f]

        # B. Motion Feature (Raw Diff -> MotionEncoder)
        diff_img = t_img - prev_img # [B*N, 3, H, W]
        motion_feat = self.motion_encoder(diff_img) # [B*N, hidden_dim, H_m, W_m]

        # 3. Alignment & Fusion (Diff2 Strategy)
        # Interpolation (if resolution mismatch between DINO & MotionEncoder)
        if src_projected.shape[-2:] != motion_feat.shape[-2:]:
            motion_feat = F.interpolate(motion_feat, size=src_projected.shape[-2:], mode='bilinear', align_corners=False)
        # Gating (Learnable Weight)
        alpha = torch.sigmoid(self.motion_logit)
        motion_feat = motion_feat * alpha
        
        # Deep Fusion (Concat -> 1x1 -> 3x3)
        assert motion_feat.shape[-2:] == src_projected.shape[-2:], (motion_feat.shape, src_projected.shape)
        fused_feat = torch.cat([src_projected, motion_feat], dim=1) 
        # Project backbone feature to hidden_dim
        transformer_input = self.fusion_proj(fused_feat) 
    
        # 4. Unfold Batch for Temporal Transformer (CDN)
        # Target Shape for CDN: [Num_Frames, B, C, H, W] (Time dimension first)
        _, C_f, H_f, W_f = transformer_input.shape # [B*N, C, H_f, W_f]
        
        # Reshape: [B*N, ...] -> [B, N, ...] -> [N, B, ...]
        transformer_input = transformer_input.view(B, num_frames, C_f, H_f, W_f).permute(1, 0, 2, 3, 4)
        
        # Unfold Mask & Pos
        mask_final = mask_t.view(B, num_frames, H_f, W_f).permute(1, 0, 2, 3)
        # pos_t is also [B*N, C, H, W]
        pos_final = pos_t[-1].view(B, num_frames, C_f, H_f, W_f).permute(1, 0, 2, 3, 4)
        ...
        # cdn_multi의 forward: src, mask, query, pos ...
        # transformer_input은 [Num_Frames, B, C, H, W]
        hopd_out, interaction_decoder_out = self.transformer(
                    transformer_input, mask_final, 
                    self.query_embed.weight, 
                    pos_final, embed_dict, targets, cur_idx)[:2]
