...
class CDN(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, class_embed_dict=None, targets=None, cur_idx=0):
        ...
        temporal_weights_dict = {} # dict for save Weight
        ...
        if self.query_temporal_interaction:
            ...
            k1 = 80 * self.num_ref_frames # step 1
            # k1 = 30 * self.num_ref_frames # step 1
            # k1 = 100 * self.num_ref_frames # step 1
            _, topk_idx1 = torch.topk(input=overall_probs, k=k1, dim=1) # [B, K1]
            ...
            # cur: [B, Q, 2C] -> [Q, B, 2C], ref: [B, K1, 2C] -> [K1, B, 2C]
            # ref: [B, K1, 2C] -> [K1, B, 2C]
            if self.num_dec_layers_temporal == 1:
                cur_concat_hs, weights_1 = self.temporal_query_layer1(
                ...
                temporal_weights_dict['layer1'] = weights_1
            else:
            ...
            # k2 = 30 * self.num_ref_frames
            # k2 = 100 * self.num_ref_frames
            _, topk_idx2 = torch.topk(input=overall_probs, k=k2, dim=1)
            ...
            # k3 = 100 * self.num_ref_frames
            _, topk_idx3 = torch.topk(input=overall_probs, k=k3, dim=1)
        ...
        # 6. Final Outputs
        # return [Batch, Num_Q, C] - only for the target frame.
        ...
        # ins_attn_weight: [Batch_size * Num_frame, Num_Q, H*W]. Slice for target.
        ...
        target_ins_attn = ins_attn_weight[target_batch_indices] # [Batch_size*Num_frame, Num_Q, H*W] -> [Batch_Size, Num_Q, H*W]
        target_rel_attn = rel_attn_weight[target_batch_indices] # [Batch_size*Num_frame, Num_Q, H*W] -> [Batch_Size, Num_Q, H*W]
        
        # reshape to spatial dimensions [B, Q, H, W]
        target_ins_attn = target_ins_attn.view(bs, self.num_queries, h, w)  # [B, Q, H*W] -> [B, Q, H, W]
        target_rel_attn = target_rel_attn.view(bs, self.num_queries, h, w)  # [B, Q, H*W] -> [B, Q, H, W]

        # memory return is usually [B, C, H, W]
        target_memory = memory[:, target_batch_indices, :].permute(1, 2, 0).view(bs, c, h, w) # [H*W, bs*num_frame, 256] -> [bs, 256, H, W]

        return final_ins_hs, final_rel_hs, target_ins_attn, target_rel_attn, target_memory, temporal_weights_dict
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.return_intermediate:
            ...
            return torch.stack(intermediate), last_attn_weight

        # return output
        return output, last_attn_weight
...
class TemporalQueryEncoderLayer(nn.Module):
    ...
    def forward(self, query, ref_query, query_pos=None, ref_query_pos=None):
        ...
        # Cross Attention. Query: Target Frame (tgt), Key/Value: Reference Frames (ref_query)
