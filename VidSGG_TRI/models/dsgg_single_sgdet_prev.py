...
class CDNDSGG(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_queries, aux_loss=False, args=None):
        super().__init__()
        print('  build similarity-driven MASKING model (threshold 0.95)')
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        # [수정] 6채널 입력 처리 (t, t-1) 및 Feature Boosting
        # samples.tensors: [Batch, Channel, H, W]

        # 1. 입력 분리 (t frame, t-1 frame)
        if samples.tensors.shape[1] == 6: # channel dimension
            # 채널 0-2: Current Frame (t)
            t_tensors = samples.tensors[:, :3, :, :]
            t_mask = samples.mask # mask는 동일함

            # # # 채널 3-5: Previous Frame (t-1)
            # prev_tensors = samples.tensors[:, 3:, :, :]
            # prev_mask = samples.mask 
            
            samples_t = NestedTensor(t_tensors, t_mask) # NestedTensor로 다시 포장 (Backbone 입력용)
            # samples_prev = NestedTensor(prev_tensors, t_mask)
            
            # 2. Backbone Forward (각각 수행)
            # features_t: List[NestedTensor], features_t[-1]이 마지막 feature map
            features_t, pos = self.backbone(samples_t)
            # features_prev, _ = self.backbone(samples_prev)
            
            # Feature Map 분해 (Batch, Channel, H_feat, W_feat)
            src_t, mask = features_t[-1].decompose()
            # src_prev, _ = features_prev[-1].decompose()
            
            # 3. Patch Similarity 계산 및 Boosting
            # DINOv2 Feature는 Norm이 안되어 있을 수 있으므로 Normalize 후 Cosine Sim 계산
            # src_t: [B, C, H, W]
            
            # # Channel 축 기준 Normalize
            feat_t_norm = F.normalize(src_t, p=2, dim=1)
            # feat_prev_norm = F.normalize(src_prev, p=2, dim=1)
            
            # # Cosine Similarity Map 계산 (Patch-wise)
            # # 결과: [B, 1, H, W] -> 각 위치의 유사도 (-1 ~ 1)
            # sim_map = torch.sum(feat_t_norm * feat_prev_norm, dim=1, keepdim=True)
            
            # # # Masking
            # threshold = 0.95 # Masking threshold
            # static_mask = sim_map >= threshold  # [B, 1, H, W], Boolean Tensor           # masked_fill을 사용하여 static_mask가 True인 위치의 Feature 값을 0으로 만듦
            
            # # Hard Masking
            # if self.is_train == 'train':
            #     # src_t는 [B, C, H, W],  mask는 [B, 1, H, W]
            #     src_masked = src_t.masked_fill(static_mask, 0.0)
            # else : 
            #     src_masked = src_t

            # Random Masking
            if  self.is_train == 'train':
                mask_prob = 0.5  # 50% masking
                random_tensor = torch.rand_like(sim_map) # # 0~1 사이 random tensor (sim_map과 동일한 device, shape)
                final_mask = static_mask & (random_tensor < mask_prob) # mask 생성
                src_masked = src_t.masked_fill(final_mask, 0.0)  # masking (0.0으로 fill
            else : 
                src_masked = src_t                
                
            # # Similarity 기반 highlighting
            # # (3) Change Map 생성 (유사도 낮음 -> 변화 큼 -> 값 큼)
            # # Range: 0 (Same) ~ 1 (Different)
            # change_map = 1 - (sim_map + 1) / 2
            # # (4) Feature Boosting
            # # 변화가 큰 영역의 Feature Magnitude를 증폭시킴
            # # alpha: boosting strength hyperparameter
            # alpha = 2.0 
            # src_boosted = src_t * (1 + alpha * change_map)
            
            # self.input_proj는 Channel 수를 맞추는 1x1 Conv
            transformer_input = self.input_proj(src_masked) # vit 출력<->transformer 입력 차원 매칭. [BS, 256, 37, 37] -> [BS, 768, 37, 37]
            
        else:
            # 기존 로직 (3채널 입력일 경우, eval 등)
            features, pos = self.backbone(samples)
            src, mask = features[-1].decompose()
            transformer_input = self.input_proj(src)  # vit 출력<->transformer 입력 차원 매칭
