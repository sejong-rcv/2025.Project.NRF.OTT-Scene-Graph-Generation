...
class CDN(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, class_embed_dict=None, targets=None, cur_idx=0):
        ...
        assert src.shape[0] == self.num_ref_frames + 1, f"got n={src.shape[0]}, expected {self.num_ref_frames+1}"
        ...
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed) # pass transformer encoder
